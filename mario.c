...
static var_t* json_parse_factor(vm_t* vm, lex_t *l) {
	if (l->tk==LEX_R_TRUE) {
		...
		//TODO
		mario_debug("Error: Can not parse json function item!\n");
		return var_new(vm);
	}
	else if (l->tk=='[') {
		/* JSON-style array */
...
var_t* new_obj(vm_t* vm, const char* name, int arg_num) {
	...
	if(n == NULL || n->var->type != V_OBJECT) {
		mario_debug("Error: There is no class: '");
		mario_debug(name);
		mario_debug("'!\n");
		return NULL;
	}
...
var_t* call_m_func_by_name(vm_t* vm, var_t* obj, const char* func_name, var_t* args) {
	...
	if(func == NULL || func->var->is_func == 0) {
		mario_debug("Interrupt function '");
...
static void do_include(vm_t* vm, const char* jsname) {
	...
	if(js == NULL) {
		mario_debug("Error: include file '");
		mario_debug(jsname);
		mario_debug("' not found!\n");
		return;
	}
		...
		switch(instr) {
				...
				if(!loaded) {
					...
					if(instr == INSTR_LOAD) {
						...
						if(n->var->type != V_OBJECT) {
							mario_debug("Warning: object or class '");
							mario_debug(s);
			...
			case INSTR_BREAK: 
			{
				while(true) {
					scope_t* sc = vm_get_scope(vm);
					if(sc == NULL) {
						mario_debug("Error: 'break' not in any loop!\n");
						vm_terminate(vm);
						break;
					}
					if(sc->is_loop) {
			...
			case INSTR_CONTINUE:
			{
				while(true) {
					scope_t* sc = vm_get_scope(vm);
					if(sc == NULL) {
						mario_debug("Error: 'continue' not in any loop!\n");
						vm_terminate(vm);
						break;
					}
					if(sc->is_loop) {
			...
			case INSTR_ASIGN: 
			{
				...
				if(modi) 
					...
					mario_debug("Can not change a const variable: '");
					mario_debug(n->name);
					mario_debug("'!\n");
				}
			...
			case INSTR_CALLO: 
			{
				...
				if(func != NULL) {
					...
					vm_push(vm, var_new(vm));
					mario_debug("Error: can not find function '");
					mario_debug(s);
					mario_debug("'!\n");
				}
				str_free(name);
			...
			case INSTR_THROW: 
			{
				while(true) {
					scope_t* sc = vm_get_scope(vm);
					if(sc == NULL) {
						mario_debug("Error: 'throw' not in any try...catch!\n");
						vm_terminate(vm);
						break;
					}
					if(sc->is_try) {
